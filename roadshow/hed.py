...
class hed:
    def __init__(self):
        self.moduleNetwork = Network().cuda().eval()

    def imagetoedge(self, image):
        height, width, channels = image.shape
        rawImage = cv2.resize(image, (480, 320), interpolation = cv2.INTER_AREA)

        tensorInput = torch.FloatTensor(numpy.array(rawImage)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
        tensorOutput = estimate(tensorInput, self.moduleNetwork)

        newImage = (tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)
        newImageUpscaled = cv2.resize(newImage, (width, height), interpolation = cv2.INTER_CUBIC)
        return newImageUpscaled
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict(torch.load('./roadshow/network-' + arguments_strModel + '.pytorch'))
    # end
